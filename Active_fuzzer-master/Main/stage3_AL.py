...
method = 'GBDT'
algorithm = 'EMCM'
stage_ip1 = '192.168.0.32'
stage_ip2 = '192.168.0.30'
...
def handle_packet(packet):
    ...
    if modify_flag == 1:
            ...
            if time.time() - modify_time <= time_interval:
                if pkt.haslayer('Raw'):
                    if pkt['IP'].src == stage_ip1 and len(pkt['Raw']) == 32:
                        rawpkt = binascii.unhexlify(reconstruct_packet(pkt,v,3,1))
                        pktnew = IP(rawpkt)
                        del pktnew['IP'].chksum
                        del pktnew['UDP'].chksum
                        packet.set_payload(str(pktnew))
                    if pkt['IP'].src == stage_ip1 and len(pkt['Raw']) == 22:
                        rawpkt = binascii.unhexlify(reconstruct_packet(pkt, v, 3, 2))
                        pktnew = IP(rawpkt)
                        del pktnew['IP'].chksum
                        del pktnew['UDP'].chksum
                        packet.set_payload(str(pktnew))
                    if pkt['IP'].src == stage_ip1 and len(pkt['Raw']) == 10:
                        rawpkt = binascii.unhexlify(reconstruct_packet(pkt, v, 3, 3))
                        pktnew = IP(rawpkt)
                        del pktnew['IP'].chksum
                        del pktnew['UDP'].chksum
                        packet.set_payload(str(pktnew))
                    if pkt['IP'].src == stage_ip2 and len(pkt['Raw']) == 22:
                        rawpkt = binascii.unhexlify(reconstruct_packet(pkt, v, 3, 4))
                        pktnew = IP(rawpkt)
                        del pktnew['IP'].chksum
                        del pktnew['UDP'].chksum
                        packet.set_payload(str(pktnew))
        ...
        if time.time() - start_time >= 5:
            vector = np.hstack((p2_50, p2_40, p2_28, p2_40))
            np.savetxt('stage3old.txt',vector)
