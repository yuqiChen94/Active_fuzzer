	...
	if accept_flag == 1:
		...
		if capture_flag == 'True':
			...
			if modify_flag == 1:
				...
				if pkt.haslayer('Raw'):
					if pkt['IP'].src == '192.168.0.22' and len(pkt['Raw']) == 32:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,2,1))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
						packet.set_payload(str(pktnew))
					if pkt['IP'].src == '192.168.0.22' and len(pkt['Raw']) == 22:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,2,2))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
						packet.set_payload(str(pktnew))
					if pkt['IP'].src == '192.168.0.22' and len(pkt['Raw']) == 10:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,2,3))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
						packet.set_payload(str(pktnew))
					if pkt['IP'].src == '192.168.0.20' and len(pkt['Raw']) == 22:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,2,4))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
				...
				if time.time() - start_time >= 5:
					vector = np.hstack((p2_50, p2_40, p2_28, p0_40))
					vector = vector.tolist()
					np.savetxt('stage2old.txt',vector,fmt='%d')
					capture_flag = 'True'
					save('capture_flag2.txt', capture_flag)
