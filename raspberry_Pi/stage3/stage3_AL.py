	...
	if accept_flag == 1:
		...
		if capture_flag == 'True':
			...
			if modify_flag == 1:
				...
				if pkt.haslayer('Raw'):
					if pkt['IP'].src == '192.168.0.32' and len(pkt['Raw']) == 32:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,3,1))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
						packet.set_payload(str(pktnew))
					if pkt['IP'].src == '192.168.0.32' and len(pkt['Raw']) == 22:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,3,2))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
						packet.set_payload(str(pktnew))
					if pkt['IP'].src == '192.168.0.32' and len(pkt['Raw']) == 10:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,3,3))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
						packet.set_payload(str(pktnew))
					if pkt['IP'].src == '192.168.0.30' and len(pkt['Raw']) == 22:
						rawpkt = binascii.unhexlify(reconstruct_packet(pkt,new_vector,3,4))
						pktnew = IP(rawpkt)
						del pktnew['IP'].chksum
						del pktnew['UDP'].chksum
				...
				if time.time() - start_time >= 5:
					vector = np.hstack((p2_50, p2_40, p2_28, p0_40))
					vector = vector.tolist()
					np.savetxt('stage3old.txt',vector,fmt='%d')
					capture_flag = 'True'
					save('capture_flag3.txt', capture_flag)
